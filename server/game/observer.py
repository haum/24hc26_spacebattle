                ...
                await self.send(
                    {
                        "type": "world_report",
                        "size": self.u.size,
                        "asteroids": [a.position for a in self.u.iter("asteroid")],
                        "vessels": [
                            (v.name(), v.position) for v in self.u.iter("vessel")
                        ],
                        "torpedos": [
                            (t.emitter().name() if t.emitter() else "", t.position)
                            for t in self.u.iter("torpedo")
                        ],
                        "mines": [
                            (m.emitter().name() if m.emitter() else "", m.position)
                            for m in self.u.iter("mine")
                        ],
                        "farmables": [f.position for f in self.u.iter("farmable")],
                    }
                )
