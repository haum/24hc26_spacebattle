...
async def set_sender(obj, reply, ws=None):
    if obj and hasattr(obj, "set_sender"):
        await obj.set_sender(functools.partial(send_replies, reply, ws=ws))
...
async def route_message(obj, reply, data, ws=None):
    ...
    if invalid:
        await send_replies(
            reply, [{"type": "invalid_msg", "errors": invalid}, "Invalid message"], ws
        )
        return

    h = getattr(obj, f"onMsg_{data['type']}", getattr(obj, "onUnknownMsg", False))
...
async def start_server():
    ...
    for d in ["doc", "2dview", "webgl2view"]:
        if os.path.isdir(d):
...
async def main():
    ...
    await asyncio.gather(*filter(lambda x: x != this_task, asyncio.all_tasks()))
